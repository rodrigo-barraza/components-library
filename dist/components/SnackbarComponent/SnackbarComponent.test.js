...
function HookHarness({ hookRef }) {
    ...
    return _jsx(SnackbarComponent, { ...result.snackbarProps });
}
describe("SnackbarComponent", () => {
    ...
    it("renders nothing when closed", () => {
        const { container } = render(_jsx(SnackbarComponent, { open: false, message: "Hello" }));
        expect(container.innerHTML).toBe("");
    });
    it("renders supporting text when open", () => {
        render(_jsx(SnackbarComponent, { open: true, message: "File saved" }));
        expect(screen.getByText("File saved")).toBeInTheDocument();
    });
    it("renders action button when actionLabel is provided", () => {
        render(_jsx(SnackbarComponent, { open: true, message: "Item deleted", actionLabel: "Undo", onAction: () => { } }));
        expect(screen.getByText("Undo")).toBeInTheDocument();
        expect(screen.getByText("Undo").tagName).toBe("BUTTON");
    });
    it("renders close button when showClose is true", () => {
        render(_jsx(SnackbarComponent, { open: true, message: "Update available", showClose: true, onDismiss: () => { } }));
        expect(screen.getByLabelText("Dismiss")).toBeInTheDocument();
    });
    it("does not render close button by default", () => {
        render(_jsx(SnackbarComponent, { open: true, message: "Hello" }));
        expect(screen.queryByLabelText("Dismiss")).not.toBeInTheDocument();
    });
    // ── Accessibility ──────────────────────────────────────
    it("has role='status' for polite announcement", () => {
        render(_jsx(SnackbarComponent, { open: true, message: "Saved" }));
    ...
    it("close button has accessible label", () => {
        render(_jsx(SnackbarComponent, { open: true, message: "Update", showClose: true, onDismiss: () => { } }));
    ...
    it("calls onAction when action button is clicked", () => {
        const spy = vi.fn();
        render(_jsx(SnackbarComponent, { open: true, message: "Deleted", actionLabel: "Undo", onAction: spy }));
        fireEvent.click(screen.getByText("Undo"));
        expect(spy).toHaveBeenCalledTimes(1);
    });
    it("triggers dismiss animation when close is clicked", () => {
        const onDismiss = vi.fn();
        render(_jsx(SnackbarComponent, { open: true, message: "Info", showClose: true, onDismiss: onDismiss }));
    ...
    it("useSnackbar shows and auto-dismisses a snackbar", () => {
        ...
        render(_jsx(HookHarness, { hookRef: hookRef }));
    ...
    it("generates unique element IDs", () => {
        render(_jsx(SnackbarComponent, { open: true, message: "Hello", actionLabel: "Action", showClose: true, onAction: () => { }, onDismiss: () => { }, id: "my-snackbar" }));
...
//# sourceMappingURL=SnackbarComponent.test.js.map