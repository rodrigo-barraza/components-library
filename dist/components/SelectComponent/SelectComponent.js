...
export default function SelectComponent({ value, options = [], onChange, placeholder = "Select...", icon = null, disabled = false, triggerTooltip = null, triggerTooltipContent = null, label = null, isOpen: controlledIsOpen, onToggle: controlledOnToggle, triggerRef: externalTriggerRef, triggerClassName, loadingProgress, onMouseEnter, children, multiple = false, allLabel = "All", compact = false, searchable = false, }) {
    ...
    const triggerClassNames = [
        ...
        .filter(Boolean)
        .join(" ");
    const triggerButton = (_jsxs("button", { ref: setTriggerRef, type: "button", className: triggerClassNames, onClick: handleToggle, onMouseEnter: onMouseEnter, disabled: disabled, style: disabled ? { cursor: "default" } : undefined, children: [_jsxs("span", { className: styles.triggerContent, children: [isLoading && (_jsx(Loader2, { size: 14, className: styles.triggerSpinner })), !isLoading && icon && _jsx("span", { className: styles.triggerIcon, children: icon }), !isLoading && !icon && !multiple && selected?.icon && _jsx("span", { className: styles.optionIcon, children: selected.icon }), multiple && !isLoading ? (allSelected ? (_jsx("span", { className: styles.triggerLabel, children: allLabel })) : compact ? (_jsxs("span", { className: styles.triggerLabel, children: [selectedOptions.length, " selected"] })) : selectedOptions.length === 0 ? (_jsx("span", { className: styles.triggerLabel, children: placeholder })) : (selectedOptions.map((option) => (_jsxs("span", { className: styles["chip-element"], children: [option.label, _jsx("button", { type: "button", className: styles["chip-remove-button"], onClick: (event) => handleRemoveChip(event, option.value), tabIndex: -1, disabled: disabled, children: _jsx(X, { size: 10 }) })] }, option.value))))) : (_jsx("span", { className: styles.triggerLabel, children: isLoading
