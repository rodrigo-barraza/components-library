...
// Reusable mock icon
const MockIcon = (props) => _jsx("svg", { "data-testid": "mock-icon", ...props });
...
describe("FabMenuComponent", () => {
    ...
    // ── Variants ────────────────────────────────────────────
    it.each(["primary", "secondary", "tertiary", "surface"])("applies the %s trigger variant class", (variant) => {
        render(_jsx(FabMenuComponent, { items: mockItems, variant: variant }));
        const trigger = screen.getByRole("button", { name: "Actions menu" });
    ...
    it("fires item onClick and closes menu when an item is clicked", async () => {
        ...
        render(_jsx(FabMenuComponent, { items: items }));
        await userEvent.click(screen.getByRole("button", { name: "Actions menu" }));
    ...
    it("disables the trigger when disabled prop is true", () => {
        render(_jsx(FabMenuComponent, { items: mockItems, disabled: true }));
        const trigger = screen.getByRole("button", { name: "Actions menu" });
        expect(trigger).toBeDisabled();
    });
    // ── Scrim ───────────────────────────────────────────────
    it("renders scrim when showScrim is true (default)", () => {
        const { container } = render(_jsx(FabMenuComponent, { items: mockItems }));
    ...
    it("does not render scrim when showScrim is false", () => {
        const { container } = render(_jsx(FabMenuComponent, { items: mockItems, showScrim: false }));
        const scrim = container.querySelector('[data-visible]');
    ...
    it("forwards ref to the container element", () => {
        ...
        render(_jsx(FabMenuComponent, { ref: ref, items: mockItems }));
        expect(ref.current).toBeInstanceOf(HTMLDivElement);
    });
    // ── Fixed positioning ───────────────────────────────────
    it("applies fixed class when fixed prop is true", () => {
        const ref = React.createRef();
        render(_jsx(FabMenuComponent, { ref: ref, items: mockItems, fixed: true }));
        expect(ref.current).toHaveClass(/fixed/i);
    });
    // ── Custom aria-label ───────────────────────────────────
    it("accepts a custom ariaLabel", () => {
        render(_jsx(FabMenuComponent, { items: mockItems, ariaLabel: "Quick actions" }));
        const trigger = screen.getByRole("button", { name: "Quick actions" });
        expect(trigger).toBeInTheDocument();
    });
});
//# sourceMappingURL=FabMenuComponent.test.js.map