 ...
 */
export function computeSunTimesMinutes(date, latitude, longitude) {
    var rad = Math.PI / 180;
    var dayOfYear = Math.floor((date.getTime() - new Date(date.getFullYear(), 0, 0).getTime()) / 86400000);
    var b = (360 / 365) * (dayOfYear - 81) * rad;
    var equationOfTime = 9.87 * Math.sin(2 * b) - 7.53 * Math.cos(b) - 1.5 * Math.sin(b);
    var declination = -23.44 * rad * Math.cos((360 / 365) * (dayOfYear + 10) * rad);
    var latitudeRad = latitude * rad;
    var cosHourAngle = (Math.sin(-0.833 * rad) - Math.sin(latitudeRad) * Math.sin(declination)) /
        (Math.cos(latitudeRad) * Math.cos(declination));
    if (cosHourAngle < -1 || cosHourAngle > 1)
        return null;
    var halfDayMinutes = (Math.acos(cosHourAngle) / rad) * 4;
    var solarNoonLocal = 720 - 4 * longitude - equationOfTime - date.getTimezoneOffset();
    return {
 ...
 */
export function autoDayWindowMinutes(date, latitude = AUTO_LATITUDE, longitude = AUTO_LONGITUDE) {
    const sun = computeSunTimesMinutes(date, latitude, longitude);
    if (sun)
        return { dayStart: sun.sunrise, dayEnd: sun.sunset };
    return { dayStart: AUTO_DAY_START_HOUR * 60, dayEnd: AUTO_DAY_END_HOUR * 60 };
}
/** Resolve what the Auto theme means at a given moment. */
export function resolveAutoTheme(date = new Date(), dayTheme = AUTO_DAY_THEME, nightTheme = AUTO_NIGHT_THEME, latitude = AUTO_LATITUDE, longitude = AUTO_LONGITUDE) {
 ...
 */
export function msUntilNextAutoBoundary(date = new Date(), latitude = AUTO_LATITUDE, longitude = AUTO_LONGITUDE) {
    ...
    if (msIntoDay < dayStart * 60000) {
        ...
        const tomorrowMidnight = new Date(date.getFullYear(), date.getMonth(), date.getDate() + 1);
