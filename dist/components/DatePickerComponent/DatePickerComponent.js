...
function MonthGrid({ year, month, from, to, hoverDate, onDayClick, onDayHover }) {
    ...
    for (let i = 0; i < firstDay; i++) {
        cells.push(_jsx("div", { className: styles['day-cell'] }, `pad-${i}`));
    }
    ...
    return (_jsxs("div", { className: styles['month-grid'], children: [_jsx("div", { className: styles['day-headers'], children: DAYS.map((dayLabel) => (_jsx("span", { className: styles['day-header'], children: dayLabel }, dayLabel))) }), _jsx("div", { className: styles['day-cells'], children: cells })] }));
}
export default function DatePickerComponent({ from = "", to = "", onChange, placeholder = "All time", storageKey = "", disabled = false, defaultOpen = false, onClose, hideTrigger = false, presets = DATE_PRESETS, showTime = true, }) {
    ...
    const updateDropdownPos = useCallback(() => {
        ...
        if (dropdownElRef.current) {
            ...
            if (left + dropRect.width > viewportWidth - 8)
                left = Math.max(8, viewportWidth - dropRect.width - 8);
            if (top + dropRect.height > viewportHeight - 8) {
                const above = rect.top - dropRect.height - 6;
                if (above >= 8)
                    top = above;
                else
                    top = Math.max(8, viewportHeight - dropRect.height - 8);
            }
        }
    ...
    return (_jsxs("div", { className: `date-picker-component ${styles['container']}`, ref: containerRef, children: [!hideTrigger && (_jsxs("button", { ref: triggerRef, type: "button", className: `${styles['trigger']} ${open ? styles['trigger-open'] : ""} ${disabled ? styles['trigger-disabled'] : ""}`, onClick: () => !disabled && setOpen((previous) => !previous), disabled: disabled, children: [_jsxs("span", { className: styles['trigger-content'], children: [_jsx("span", { className: styles['trigger-icon'], children: _jsx(Calendar, { size: 13 }) }), _jsx("span", { className: styles['trigger-text'], children: displayText || placeholder })] }), hasValue ? (_jsx("span", { className: styles['trigger-clear'], onClick: (event) => { event.stopPropagation(); handleClear(); }, title: "Clear dates", children: _jsx(X, { size: 12 }) })) : (_jsx(ChevronDown, { size: 14, className: `${styles['chevron']} ${open ? styles['chevron-open'] : ""}` }))] })), hideTrigger && _jsx("div", { ref: triggerRef }), open && (_jsxs("div", { ref: dropdownElRef, className: styles['dropdown'], style: { top: dropdownPos.top, left: dropdownPos.left }, children: [_jsx("div", { className: styles['presets'], children: presets.map((preset) => {
                            const isActive = getActiveDatePreset(from, to) === preset.label;
                            return (_jsx("button", { type: "button", className: `${styles['preset-button']} ${isActive ? styles['preset-btn-active'] : ""}`, onClick: () => handlePreset(preset), children: preset.label }, preset.label));
                        }) }), _jsxs("div", { className: styles['calendars'], children: [_jsxs("div", { className: styles['month-nav'], children: [_jsx("button", { type: "button", className: styles['month-nav-button'], onClick: prevMonth, children: _jsx(ChevronLeft, { size: 14 }) }), _jsx("span", { className: styles['month-label'], children: monthLabel(viewDate.year, viewDate.month) }), _jsx("span", { className: styles['month-label'], children: monthLabel(secondMonth.year, secondMonth.month) }), _jsx("button", { type: "button", className: styles['month-nav-button'], onClick: nextMonth, children: _jsx(ChevronRight, { size: 14 }) })] }), _jsxs("div", { className: styles['month-pair'], children: [_jsx(MonthGrid, { year: viewDate.year, month: viewDate.month, from: activeFrom, to: activeTo, hoverDate: hoverDate, onDayClick: handleDayClick, onDayHover: setHoverDate }), _jsx(MonthGrid, { year: secondMonth.year, month: secondMonth.month, from: activeFrom, to: activeTo, hoverDate: hoverDate, onDayClick: handleDayClick, onDayHover: setHoverDate })] }), selecting && (_jsx("div", { className: styles['select-hint'], children: "Click a second date to complete the range" })), showTime && showTimeRow && !selecting && (_jsxs("div", { className: styles['time-row'], children: [_jsxs("div", { className: styles['time-field'], children: [_jsx(Clock, { size: 12, className: styles['time-icon'] }), _jsx("span", { className: styles['time-label'], children: "From" }), _jsx("input", { type: "time", className: styles['time-input'], value: fromTime, onChange: (event) => setFromTime(event.target.value) })] }), _jsx("span", { className: styles['time-sep'], children: "\u2013" }), _jsxs("div", { className: styles['time-field'], children: [_jsx(Clock, { size: 12, className: styles['time-icon'] }), _jsx("span", { className: styles['time-label'], children: "To" }), _jsx("input", { type: "time", className: styles['time-input'], value: toTime, onChange: (event) => setToTime(event.target.value) })] }), _jsx("button", { type: "button", className: styles['time-apply-button'], onClick: handleApplyTime, children: "Apply" })] }))] })] }))] }));
}
