...
export function generateThemeInitScript(storageKey, validThemes = ["light", "dark", "tropical", "oceanic", "punk"], customThemesKey) {
    ...
    return `(function(){try{var r=localStorage.getItem(${JSON.stringify(storageKey)});if(r){var t=JSON.parse(r);if(${themeList}.indexOf(t)!==-1||t.indexOf("custom-")===0){document.documentElement.setAttribute("data-theme",t)}}}catch(e){}try{var c=localStorage.getItem(${JSON.stringify(customKey)});if(c){var themes=JSON.parse(c);if(Array.isArray(themes)){themes.forEach(function(th){if(th&&th.id&&th.tokens){var s=document.createElement("style");s.id="custom-theme-"+th.id;s.setAttribute("data-custom-theme",th.id);var tk=th.tokens;var isL=(function(h){var hex=h.replace("#","");var r2=parseInt(hex.slice(0,2),16),g2=parseInt(hex.slice(2,4),16),b2=parseInt(hex.slice(4,6),16);return(r2*299+g2*587+b2*114)/1000>128})(tk.bgPrimary||"#000");var sel='[data-theme="custom-'+th.id+'"]';s.textContent=sel+"{--accent-color:"+tk.accentColor+";--accent-secondary:"+(tk.accentSecondary||tk.accentColor)+";--bg-primary:"+tk.bgPrimary+";--bg-secondary:"+tk.bgSecondary+";--bg-tertiary:"+tk.bgTertiary+";--text-primary:"+tk.textPrimary+";--text-secondary:"+tk.textSecondary+";--text-tertiary:"+tk.textTertiary+";--border-color:"+(isL?"rgba(0,0,0,0.1)":"rgba(255,255,255,0.06)")+";--danger:"+tk.danger+";--success:"+tk.success+";--warning:"+tk.warning+";--info:"+tk.info+"}";document.head.appendChild(s)}})}}catch(e2){}})();`;
}
