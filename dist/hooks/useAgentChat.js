...
const DEFAULT_ERROR_MESSAGE = "Something went wrong while generating a response. Please try again.";
function pickRandom(items) {
    return items[Math.floor(Math.random() * items.length)];
}
export default function useAgentChat(options) {
    const { greeting, offlineFallbacks, simulatedDelayMs, onError, ...serviceConfig } = options;
    ...
    const service = useMemo(() => new AgentChatService(configRef.current), 
    ...
    useEffect(() => {
        if (hasGreeted || !greeting || messages.length > 0)
            return;
        setIsStreaming(true);
    ...
    const upsertToolCall = useCallback((id, toolCall) => {
        setMessages((previous) => previous.map((message) => {
            ...
            const nextToolCalls = index === -1
                ? [...existing, toolCall]
                : existing.map((tc, tcIndex) => tcIndex === index ? { ...tc, ...toolCall } : tc);
    ...
    const stop = useCallback(() => {
        ...
        setMessages((previous) => previous.map((message) => {
            ...
            const toolCalls = rest.toolCalls?.map((toolCall) => toolCall.status === AGENT_TOOL_CALL_STATUS.RUNNING
