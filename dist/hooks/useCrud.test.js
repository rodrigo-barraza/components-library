...
describe("useCrud", () => {
    it("creates and prepends to list", async () => {
        ...
        const { result } = renderHook(() => useCrud({ createFn, updateFn: vi.fn(), removeFn: vi.fn() }));
        let created;
        await act(async () => {
    ...
    it("updates item in list by id", async () => {
        ...
        const { result } = renderHook(() => useCrud({ createFn: vi.fn(), updateFn, removeFn: vi.fn() }));
    ...
    it("removes item from list by id", async () => {
        ...
        const { result } = renderHook(() => useCrud({ createFn: vi.fn(), updateFn: vi.fn(), removeFn }));
        act(() => {
...
//# sourceMappingURL=useCrud.test.js.map