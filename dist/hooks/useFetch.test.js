...
describe("useFetch", () => {
    ...
    it("re-fetches when deps change", async () => {
        const fetcher = vi.fn().mockResolvedValue("data");
        let dep = "a";
        const { result, rerender } = renderHook(() => useFetch(fetcher, { deps: [dep] }));
        await vi.waitFor(() => {
            expect(result.current.loading).toBe(false);
        });
    ...
    it("applies transform function", async () => {
        ...
        const { result } = renderHook(() => useFetch(fetcher, { transform: (d) => d.items.length }));
        await vi.waitFor(() => {
...
//# sourceMappingURL=useFetch.test.js.map